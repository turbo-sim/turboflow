...
def print_config_summary(config: dict, default_dict):

    print("Configuration Summary:")
    for key, value in config.items():
        default_value = default_dict[key]
        is_default = value == default_value
        print(f"{key}: {value} (default: {default_value}) {'[default used]' if is_default else '[custom value]'}")



# Function to load and validate the configuration file
def load_config(config_file_path: str, mode = "performance_analysis", print_summary = True):
    try:
