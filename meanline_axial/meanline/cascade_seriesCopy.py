...
def evaluate_cascade_series(
    variables_values, BC, geometry, model_options, reference_values
):
    # Load variables
    ...
    for i in range(n_cascades):
        ...
        geometry_cascade = {
            key: values[i] for key, values in geometry.items() if key != "n_cascades"
        }
        ...
        if i != n_cascades - 1:
            (
                ...
                cascades_data["plane"]["h0"].values[-1],
    ...
    cascades_data["overall"]["eta_ts"] = eta_ts
    cascades_data["overall"]["eta_tt"] = (
    ...
    cascades_data["overall"]["pr_tt"] = (
        BC["p0_in"] / cascades_data["plane"]["p0"].values[-1]
    )
    cascades_data["overall"]["pr_ts"] = (
        BC["p0_in"] / cascades_data["plane"]["p"].values[-1]
    )
