...
def compute_performance(
    ...
    for i, operation_point in enumerate(operation_points):
        ...
        try:
            ...
            solution_data.append(scale_to_real_values(solver.solution.x, case_data))
...
def get_initial_guess(cascades_data, initial_guess = None):
    ...
    initial_guess = scale_to_normalized_values(initial_guess, cascades_data)
...
def compute_initial_guess(cascades_data, R, eta_tt, eta_ts, Ma_crit):
    """
    ...
    if n_stages != 0: 
        ...
        for i in range(n_stages):
            ...
            h02 = h01
            v2 = np.sqrt(2*(h02-h2))
            vel2_data = cs.evaluate_velocity_triangle_in(u, v2, alpha2)
            w2 = vel2_data["w"]
            h0rel2 = h2 + 0.5*w2**2
            ...
            vel3_data = cs.evaluate_velocity_triangle_out(u, w3, beta3)
            v3 = vel3_data["v"]
            vm3 = vel3_data["v_m"]
            h03 = h3 + 0.5*v3**2
...
def compute_single_operation_point(
    ...
    initial_guess = get_initial_guess(cascades_data, initial_guess)
    ...
    if not success:
        ...
        for i in range(N):
            ...
            print_dict(x0)
            initial_guess = get_initial_guess(cascades_data, x0)
            solver = initialize_solver(cascades_data, problem, method, initial_guess)
            try:
                solution = solver.solve()
                success = solution.success
...
class CascadesNonlinearSystemProblem(NonlinearSystemProblem):
    def __init__(self, cascades_data):
        ...
        cascades_data["fluid"] = FluidCoolProp_2Phase(cascades_data["operation_points"]["fluid_name"])
