...
# Create logger
logger = tf.create_logger(name="R125", path=f"output/logs", use_datetime=True, to_console=True)
...
if MODE == "performance_analysis":
    ...
    operation_points = config["operation_points"]
    solver = tf.compute_optimal_turbine(config, export_results=False, logger = logger)
