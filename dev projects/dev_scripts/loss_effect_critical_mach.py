...
if interpolation:
    ...
    f = (
        lambda x: coeff_mach_crit[0] * x**4
        + coeff_mach_crit[1] * x**3
        + coeff_mach_crit[2] * x**2
        + coeff_mach_crit[3] * x
        + coeff_mach_crit[4]
    )
    coeff_phi_max = np.polyfit(Y, phi_max, 4)
    f1 = (
        lambda x: coeff_phi_max[0] * x**4
        + coeff_phi_max[1] * x**3
        + coeff_phi_max[2] * x**2
        + coeff_phi_max[3] * x
        + coeff_phi_max[4]
...
if plot_data:
    ...
    for i in range(len(Y)):
        ...
        ax.plot(
            data_subset["Mach"],
            data_subset["Mass flux"] / phi_max,
            label=f"Y = {Y[i]:.1f}",
            color=colors[i],
            linestyle=linestyles[i],
        )
        ax1.plot(
            data_subset["Pressure ratio"],
            data_subset["Mach"],
            label=f"Y = {Y[i]:.1f}",
            color=colors[i],
            linestyle=linestyles[i],
    ...
    if interpolation:
        ...
        ax.plot(
            mach_crit_iterp, f2(mach_crit_iterp) / phi_max, color="k", linestyle=":"
        )
...
if plot_mass_flow:
    ...
    for i in range(len(Y)):
        ...
        mass_flow_crit = (
            np.concatenate((np.ones(n) * phi_max[i], mass_flow_crit)) * Area
        )
        mass_flow_1 = np.concatenate((np.ones(n2) * phi_Ma_1[i], mass_flow_1)) * Area
        ax.plot(
            1 / pressure_ratio,
            mass_flow_crit / phi_max[0] / Area,
            color=colors[i],
            linestyle="-",
            label=f"Y = {Y[i]:.1f}",
        )
        ax.plot(
            1 / pressure_ratio,
            mass_flow_1 / phi_max[0] / Area,
            color=colors[i],
            linestyle="--",
        )

    ax.set_ylabel(
        "Normalized mass flow rate [$\dot{m}/\dot{m}_\mathrm{isentropic, max}$]"
    )
    ax.set_xlabel(
        "Total-to-static pressure ratio [$p_{0, \mathrm{in}}/p_\mathrm{out}$]"
    )
    ax.set_xlim([1.0, 3.5])
