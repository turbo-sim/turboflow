...
class AxialTurbineProblem(psv.NonlinearSystemProblem):
    ...
    def update_boundary_conditions(self, operation_point):
        ...
        # state_in_stag = self.fluid.get_state(cp.PT_INPUTS, p0_in, T0_in)

        # state_in_stag = tf.get_state_custom_jvp(self.fluid, cp.PT_INPUTS, p0_in, T0_in)
        ...
        # state_out_s = self.fluid.get_state(cp.PSmass_INPUTS, p_out, s_in)

        # state_out_s = tf.get_state_custom_jvp(self.fluid, cp.PSmass_INPUTS, p_out, s_in)
        ...
        # state_out_h = self.fluid.get_state(cp.HmassP_INPUTS, h0_in, p_out)
...
def get_heuristic_guess(
    ...
    # stag_first = fluid.get_state(cp.PT_INPUTS, p0_first, T0_first)

    # stag_first = tf.get_state_custom_jvp(fluid, cp.PT_INPUTS, p0_first, T0_first)
    ...
    # static_is = fluid.get_state(cp.PSmass_INPUTS, p_final, s_first)

    # static_is = tf.get_state_custom_jvp(fluid, cp.PSmass_INPUTS, p_final, s_first)
    ...
    # static_properties_exit = tf.get_state_custom_jvp(fluid, cp.HmassP_INPUTS, h_final, p_final)

    # static_is = tf.get_state_custom_jvp(cp.PSmass_INPUTS, p_final, s_first)
    ...
    for i in range(number_of_cascades):
        ...
        # static_out = fluid.get_state(cp.PSmass_INPUTS, p_out, s_out)

        # static_out = tf.get_state_custom_jvp(fluid, cp.PSmass_INPUTS, p_out, s_out)
        ...
        # static_props_is = fluid.get_state(cp.PSmass_INPUTS, p_out, s_in)

        # static_props_is = tf.get_state_custom_jvp(fluid, cp.PSmass_INPUTS, p_out, s_in)
        ...
        s_throat_crit = s_out
        # static_state_throat_crit = fluid.get_state(cp.HmassSmass_INPUTS, h_throat_crit, s_throat_crit)

        # static_state_throat_crit = tf.get_state_custom_jvp(fluid, cp.HmassSmass_INPUTS, h_throat_crit, s_throat_crit)
        ...
        if i != (number_of_cascades - 1):
            ...
            s_in = s_out
            # static_in = fluid.get_state(cp.HmassSmass_INPUTS, h_in, s_in)

            # static_in = tf.get_state_custom_jvp(fluid, cp.HmassSmass_INPUTS, h_in, s_in)
