...
from turboflow.axial_turbine import performance_analysis as pa

from turboflow.pysolver_view import numerical_differentiation
from turboflow.pysolver_view import optimization_wrappers as _opt
from turboflow.pysolver_view.pysolver_utilities import savefig_in_formats
...
def fitness_gradient(config):
    ...
    grad_FD = numerical_differentiation.approx_gradient(
                problem.fitness,
                x,
                f0=problem.fitness(x),
                method="2-point",
                abs_step=None,  ## TODO make sure it works when design variable takes value 0 * np.abs(x),
            )

    return fitness_func, x, x_keys, grad_jax, grad_FD
...
class CascadesOptimizationProblem(psv.OptimizationProblem):
    ...
    def fitness_gradient_based(self, x):
        ...
        self.updated_geometry = self.geometry
        ...
        # Evaluate objective function
        self.f = self.get_nested_value(self.results, self.obj_func["variable"])/self.obj_func["scale"] # self.obj.func on the form "key.column"

        # Evaluate additional constraints
        self.results["additional_constraints"] = pd.DataFrame({"interspace_area_ratio": self.geometry["A_in"][1:]
            / self.geometry["A_out"][0:-1]})
        
        self.results["additional_constraints"] = {"interspace_area_ratio": self.updated_geometry["A_in"][1:] 
            / self.updated_geometry["A_out"][0:-1]}
        
        # Evaluate constraints
        self.c_eq1 = jnp.array(list(self.results["residuals"].values()))
        self.c_eq = jnp.append(self.c_eq1, self.evaluate_constraints(self.eq_constraints))
    ...
    def evaluate_constraints(self, constraints_list):
        ...
        for constraint in constraints_list:
            updated_constraints = jnp.append(
                constraints,
