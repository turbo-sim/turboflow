...
#     # Inlet state (static) and components
#     p_in = operating_conditions.p_in
#     h_in = operating_conditions.h_in
#     v_in = operating_conditions.v_in
#     alpha_in = operating_conditions.alpha_in  # degrees

#     v_m_in = v_in * jnp.cos(jnp.deg2rad(alpha_in))
#     v_t_in = v_in * jnp.sin(jnp.deg2rad(alpha_in))

#     # Print inlet (what we actually pass to the channel)
#     jdbg.print(
#         "[chan-in] p_in={:.3f} Pa  h_in={:.3f} J/kg  v_in={:.3f} m/s  alpha_in={:.3f} deg  -> vm={:.3f}, vt={:.3f}",
#         p_in, h_in, v_in, alpha_in, v_m_in, v_t_in
#     )

#     # Geometry sanity at s=0
#     g0 = geom_handle(s=0.0)
#     jdbg.print(
#         "[geom0] r0={:.6f} m  b0={:.6f} m  A0={:.6f} m^2  phi0={:.3f} deg",
#         g0["r"], g0["b"], g0["A"], g0["phi"]
#     )

#     # Inlet stagnation quantities derived from (h_in, p_in, v_in)
#     state_in = fluid.get_state(jxp.HmassP_INPUTS, h_in, p_in)
#     h0_in = h_in + 0.5 * v_in**2
#     s0_in = state_in.s
#     p0_in = fluid.get_state(jxp.HmassSmass_INPUTS, h0_in, s0_in).p
...
#         p_in,     # 3: static pressure [Pa]
#         0.0,      # 4: η (accumulator)
#         1.0,      # 5: η_kinetic (starts at 1)
#         0.0,      # 6: η_loss
#         0.0,      # 7: η_loss_wall
#         0.0,      # 8: η_loss_diff
#         0.0,      # 9: η_loss_curv
#         s0_in,    # 10: integrated entropy (start at s_in)
#         0.0,      # 11: theta
#     ])
...
#         "h_in": h_in,
#         "v_in": v_in,
#         "alpha_in": alpha_in,   # degrees (by design)
#         "p0_in": p0_in,
#         "h0_in": h0_in,
#         "s0_in": s0_in,
#         "m_total": m_total,
...
#     out = solution.ys  # dict of arrays (from func_all)

#     # First-step diagnostics (arrays indexed on saved ts)
#     p = out["p"]; r = out["r"]; A = out["A"]
#     vm = out["v_m"]; vt = out["v_t"]; alpha_used = out["alpha"]

#     # Guard if n_points < 2
#     def _print_two():
#         jdbg.print(
#             "[chan-step1] p0={:.3f}  p1={:.3f}  dp={:.3f}   r0={:.6f}->{:.6f}   A0={:.6f}->{:.6f}",
#             p[0], p[1], p[1] - p[0], r[0], r[1], A[0], A[1]
#         )
#         jdbg.print(
#             "[chan-step1] vm0={:.3f} vt0={:.3f}  alpha_used0={:.3f} deg   vm1={:.3f} vt1={:.3f}  alpha_used1={:.3f} deg",
#             vm[0], vt[0], alpha_used[0], vm[1], vt[1], alpha_used[1]
#         )
#         return 0
...
#     jdbg.print(
#         "[chan-range] p_min={:.3f}  p_max={:.3f}  (should stay > 0)",
#         jnp.min(p), jnp.max(p)
#     )
...
def evaluate_vaneless_channel_ode(t, y, args):
    ...
    #     m_coord,Re, b, A, dAds, params["alpha_in"], params["b_in"], m_total, curvature, alpha
    # )
...
#     (v_m, v_t, h, p,
#      eta, efficiency_kinetic, efficiency_loss,
#      efficiency_loss_wall, efficiency_loss_diff, efficiency_loss_curv,
#      s_int, theta) = y

#     # Always get geometry first (safe)
#     geom = geom_handle(m_coord)
#     r   = geom["r"]
#     b   = geom["b"]
#     A   = geom["A"]
...
#     # If pressure is about to go invalid, dump and short-circuit
#     def _emergency(_):
#         jdbg.print(
#             "[ode-bad] t={:.6e}  p={:.3f}  h={:.3f}  vm={:.3f}  vt={:.3f}  |  r={:.6f}  b={:.6f}  A={:.6f}  dAds={:.3e}  phi={:.3f}",
#             t, p, h, v_m, v_t, r, b, A, dAds, phi
#         )
#         rhs = jnp.zeros_like(y)
#         out = {
#             **geom,
#             "m": t,
#             "v_t": v_t, "v_m": v_m, "v": jnp.sqrt(jnp.maximum(v_t,0.0)**2 + jnp.maximum(v_m,0.0)**2),
#             "alpha": jnp.rad2deg(jnp.arctan2(v_t, jnp.maximum(v_m,1e-9))),
#             # placeholders so SaveAt(fn=...) keeps working
#             "Ma": 0.0, "Ma_m": 0.0, "Ma_t": 0.0,
#             "Re": 0.0, "mu": 0.0,
#             "p": p, "T": 0.0, "d": 0.0, "s": 0.0, "s_int": s_int,
#             "h": h, "h_s": 0.0, "h0": h, "p0": p, "T0": 0.0, "d0": 0.0,
#             "theta": theta,
#             "Cp": 0.0, "KE": 0.0, "Y": 0.0,
#             "efficiency": eta,
#             "efficiency_kinetic": efficiency_kinetic,
...
#             "efficiency_loss_diffusion": efficiency_loss_diff,
#             "efficiency_loss_curvature": efficiency_loss_curv,
#             "cf_total": 0.0, "cf_wall": 0.0, "cf_diffusion": 0.0, "cf_curvature": 0.0,
#             "E_diffusion": 0.0, "q_w": 0.0, "htc": 0.0,
#         }
#         return rhs, out

#     def _normal(_):
#         # Kinematics
#         v = jnp.sqrt(jnp.maximum(v_t, 0.0)**2 + jnp.maximum(v_m, 0.0)**2)
#         alpha = jnp.rad2deg(jnp.arctan2(v_t, jnp.maximum(v_m, 1e-12)))

#         # Thermo states (this is where negative p would crash, hence the guard above)
#         state = fluid.get_state(jxp.HmassP_INPUTS, h, p)
#         a  = state["a"]
#         d  = state["d"]
#         T  = state["T"]
...
#         # Skin-friction and diffusion/curvature
#         cf_wall, cf_diff, cf_curv, E = model_options.friction.get_cf_components(
#             m_coord=m_coord,
#             Re=Re,
#             b=b,
#             A=A,
#             dA_dm=dAds,
#             alpha_in=params["alpha_in"],  # degrees
#             b_in=params["b_in"],
#             m_total=m_total,
#             curvature=curvature,
#             alpha=alpha,                   # degrees
...
#         # --- NEW: build a bounded total Cf for shear stresses
#         Cf_total = cf_wall + cf_diff + cf_curv
#         # keep within a physically plausible band
#         Cf_total = jnp.clip(Cf_total, 0.0, 0.08) 

#         # Shear stress components (Aungier split)
#         # ORIGINAL-A-like split but with bounded Cf_total
#         tau_m = 0.5 * d * v**2 * (Cf_total * jnp.cos(jnp.deg2rad(alpha)))
#         tau_t = 0.5 * d * v**2 * (Cf_total * jnp.sin(jnp.deg2rad(alpha)))


#         # Heat transfer
#         q_w, htc = model_options.heat_transfer.compute_heat_transfer(
#             T0_fluid=T0, rho=d, v=v, cp=state["cp"], k=state["conductivity"],
#             mu=mu, Cf=cf_wall, Dh=D_h,
#         )
...
#         S = jnp.asarray([
#             -d * (dAds / A),
#              d * v_t**2 / r * sin_phi - 2.0 * tau_m / b,
#             -d * v_t * v_m / r * sin_phi - 2.0 * tau_t / b,
#              (2.0 / b) * q_w,
#         ])

#         dv_m, dv_t, dh, dp = jnp.linalg.solve(M, S)

#         # One-time deep snapshot near the inlet
#         def _log_at_start(_):
#             # Scalars with format specs are fine
#             jdbg.print("[ode@0] r={:.6f} b={:.6f} A={:.6f} dAds={:.3e} phi={:.3f}",
#                     r, b, A, dAds, phi)
#             jdbg.print("[ode@0] d={:.3f} a={:.3f} G={:.3e} mu={:.3e} Re={:.2f}",
#                     d, a, G, mu, Re)
#             jdbg.print("[ode@0] vt={:.3f} vm={:.3f} v={:.3f} alpha={:.3f}",
#                     v_t, v_m, v, alpha)
#             jdbg.print("[ode@0] cf_w={:.3e} cf_d={:.3e} cf_c={:.3e} | tau_m={:.3e} tau_t={:.3e}",
#                     cf_wall, cf_diff, cf_curv, tau_m, tau_t)

#             # Arrays: NO format spec — use "{}"
#             jdbg.print("[ode@0] M rows:")
#             jdbg.print("  M[0]={}", M[0])
#             jdbg.print("  M[1]={}", M[1])
#             jdbg.print("  M[2]={}", M[2])
#             jdbg.print("  M[3]={}", M[3])

#             jdbg.print("[ode@0] S={}", S)

#             # Extra scalars that help diagnose pressure drop direction
#             jdbg.print("[ode@0] rhs: dvm={:.3e} dvt={:.3e} dh={:.3e} dp={:.3e}",
#                     dv_m, dv_t, dh, dp)
#             jdbg.print("[ode@0] p={:.3f} h={:.3f} h0={:.3f} s={:.6f}", p, h, h0, s)

#             return jnp.int32(0)
        
#         _ = jax.lax.cond(t < 1e-9, _log_at_start, lambda _: jnp.int32(0), operand=None)

#         # Entropy generation & wrapping
#         s_gen = 2.0 / b * (tau_m * v_m + tau_t * v_t)
#         ds_int = s_gen / (d * v_m * T)
#         dtheta = (v_t / v_m) / r

#         # Efficiency derivatives
#         dEff, dEff_kin, dEff_loss, (dEff_wall, dEff_diff, dEff_curv) = compute_efficiency_derivatives(
#             dh, dp, d, alpha, params["h0_in"], params["h_in"], cf_wall, cf_diff, cf_curv
#         )

#         # Pressure coefficients
#         p_in = params["p_in"]; p0_in = params["p0_in"]
#         Cp = (p - p_in) / (p0_in - p_in)
#         KE = (p0 - p)  / (p0_in - p_in)
#         Y  = (p0_in - p0) / (p0_in - p_in)

#         rhs_core  = jnp.array([dv_m, dv_t, dh, dp])
#         rhs_eff   = jnp.array([dEff, dEff_kin, dEff_loss, dEff_wall, dEff_diff, dEff_curv])
#         rhs_extra = jnp.asarray([ds_int, dtheta])
#         rhs = jnp.concatenate([rhs_core, rhs_eff, rhs_extra])
...
#             **geom,
#             "m": t,
#             "v_t": v_t, "v_m": v_m, "v": v, "alpha": alpha,
#             "Ma": v / a, "Ma_m": v_m / a, "Ma_t": v_t / a,
#             "Re": Re, "mu": mu,
#             "p": p, "T": T, "d": d, "s": s, "s_int": s_int,
#             "h": h, "h_s": h_s, "h0": h0, "p0": p0, "T0": T0, "d0": d0,
#             "theta": theta,
#             "Cp": Cp, "KE": KE, "Y": Y,
#             "efficiency": eta,
#             "efficiency_kinetic": efficiency_kinetic,
...
#             "efficiency_loss_curvature": efficiency_loss_curv,
#             "cf_total": cf_wall + cf_diff + cf_curv,
#             "cf_wall": cf_wall, "cf_diffusion": cf_diff, "cf_curvature": cf_curv,
#             "E_diffusion": E, "q_w": q_w, "htc": htc,
#         }
#         return rhs, out

#     return jax.lax.cond(p > 1.0e3, _normal, _emergency, operand=None)
