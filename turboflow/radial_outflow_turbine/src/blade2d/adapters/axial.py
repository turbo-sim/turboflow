"""Axial geometry adapter for curvature-based camberline generation."""
...
def curvature_camberline_from_axial_geometry(
    ...
    *,
    ...
    """Build curvature-based camberline from an axial geometry dictionary.
    ...
    Optional:
    - chord (m)
    - chord_axial / meridional_chord (projection in m)
    - cascade_type
    """
    ...
    chord_projection = geometry.get("chord_axial", geometry.get("meridional_chord", None))
    ...
    chord_value, chord_source = resolve_axial_chord(
        geometry,
...
def thickness_profile_from_axial_geometry(
    geometry: dict,
    *,
    n_points: int = 161,
    n_control: int = 11,
    ...
    """Build quartic B-spline thickness profile from axial geometry dictionary."""
    if u is None:
        u = jnp.linspace(0.0, 1.0, n_points, dtype=jnp.float64)
...
def section_from_axial_geometry(
    geometry: dict,
    *,
    n_points: int = 161,
    ...
    """Generate a complete 2D section (top/bottom/camber) from axial geometry dict."""
    camber = curvature_camberline_from_axial_geometry(
        geometry,
        n_points=n_points,
    ...
    if camber.get("chord", None) is None:
        raise KeyError(
            "Unable to resolve axial chord for section construction. "
            "Provide one of: 'chord', 'chord_axial', or 'meridional_chord'."
        )
    thickness = thickness_profile_from_axial_geometry(
        geometry,
        n_points=n_points,
