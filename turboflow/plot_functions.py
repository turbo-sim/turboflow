...
def plot_axial_radial_plane(geometry, linestyle = "-", blade_color = "k", plot_rotation_ax = True, plot_casing = True, label = "", fig = None, ax = None):
    r"""
    ...
    if fig == None:
        ...
        ax.set_aspect("equal")
        ax.set_ylim([0, 1.5 * max(y_tip) * ct + max(y_tip)])
    ...
    for i in range(number_of_cascades):
        if i == 0:
            ax.plot(x[i * 2 : (i + 1) * 2], y_tip[i * 2 : (i + 1) * 2], color = blade_color, linestyle = linestyle, label = label)  # Plot tip
        else: 
            ax.plot(x[i * 2 : (i + 1) * 2], y_tip[i * 2 : (i + 1) * 2], color = blade_color, linestyle = linestyle)  # Plot tip
        ax.plot(x[i * 2 : (i + 1) * 2], y_hub[i * 2 : (i + 1) * 2], color = blade_color, linestyle = linestyle)  # Plot hub
        ax.plot(
...
def plot_view_axial_tangential(turbine_data, 
                               geometry, 
                               fig = None, 
                               ax = None, 
                               repeats = 2,
    ...
    reaction = np.array(turbine_data['stage']['reaction'].values)  # one per stage
