...
if Case == "pressure_line":
    ...
    ax1.text(
        Ma_crit_throat - 0.02, 67.01 - 0.1, "$\mathrm{Ma}^*$", fontsize=13, color="k"
    )
    ax1.text(
        Ma_crit_throat + 0.03, 67.01 + 0.25, "Cascade chokes", fontsize=13, color="k"
    )
    ...
    ax1.add_patch(arrow1)
    ax1.text(
        Ma_crit_throat - 0.1, beta_g + 0.21, "Zero deviation", fontsize=13, color="k"
    )
