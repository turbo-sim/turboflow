...
for j in range(len(Y)):
    for i in range(N):
        ...
        sol = optimize.root_scalar(calculate_residual, args = (p01, h01, Y[j], v0), bracket = [0.01, 0.999])
        v2 = sol.root*v0
        h2 = h01 - 0.5*v2**2
        fluid.update(cp.HmassP_INPUTS, h2, p2)
...
if plot_figures:
    ...
    ax3.set_xlabel('Total-to-static pressure ratio [$p_\mathrm{out}/p_{0, \mathrm{in}}$]')
