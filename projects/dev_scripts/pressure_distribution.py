...
import meanline_axial.axial_turbine.geometry_model as geom
from scipy.optimize import root_scalar
  
# Define auxiliaty functions
def get_enthalpy_residual_inlet(p_in, h0_in, s_in, mass_flow, area_in):
    state_in = Fluid.set_state(ml.PSmass_INPUTS, p_in, s_in)
...
def get_enthalpy_residual_outlet(p_out, h0_rel_in, s_in, mass_flow, area_out, angle_out):
    state_out = Fluid.set_state(ml.PSmass_INPUTS, p_out, s_in)
    velocity_out = mass_flow / (state_out.d * area_out * ml.cosd(angle_out))
...
# Inlet state
Fluid = ml.Fluid(fluid_name)
state_01 = Fluid.set_state(ml.PT_INPUTS, p0_in, T0_in)
...
for i, mass_flow in enumerate(mass_flows):
    ...
    p_1 = result.root
    state_1 = Fluid.set_state(ml.PSmass_INPUTS, p_1, state_01.s)
    ...
    p_2 = result.root
    state_2 = Fluid.set_state(ml.PSmass_INPUTS, p_2, state_01.s)
    u_2 = omega * radius
    v_2 = mass_flow / (state_2.d * area * ml.cosd(angle))
    v_t_2 = v_2 * ml.sind(angle)
    v_m_2 = v_2 * ml.cosd(angle)
    w_t_2 = v_t_2 - u_2
    w_m_2 = v_m_2
    ...
    p_2_array[i] = p_2
    Ma_2_array[i] = Ma_2
    state_01 = Fluid.set_state(ml.HmassSmass_INPUTS, h0rel_in, state_01.s)
    d_crit = state_01.d * ((gamma + 1) / 2) ** (-1 / (gamma-1))
    a_crit = state_01.a * ((gamma + 1) / 2) ** (-0.5)
    mass_flux_crit_1[i] = d_crit*a_crit
    mass_flow_crit_1[i] = d_crit*a_crit*area*ml.cosd(angle)
    ...
    p_3 = result.root
    state_3 = Fluid.set_state(ml.PSmass_INPUTS, p_3, state_01.s)
    u_3 = omega * radius
    w_3 = mass_flow / (state_3.d * area * ml.cosd(angle))
    w_t_3 = w_3 * ml.sind(angle)
    w_m_3 = w_3 * ml.cosd(angle)
    v_t_3 = w_t_3 + u_3
    v_m_3 = w_m_3
    ...
    state_02rel = Fluid.set_state(ml.HmassSmass_INPUTS, h0rel_in, state_01.s)
    d_crit = state_02rel.d * ((gamma + 1) / 2) ** (-1 / (gamma-1))
    a_crit = state_02rel.a * ((gamma + 1) / 2) ** (-0.5)
    mass_flux_crit_2[i] = d_crit*a_crit
    mass_flow_crit_2[i] = d_crit*a_crit*area*ml.cosd(angle)
    ...
    if two_stage:
        ...
        result = root_scalar(res, x0=p0_in*0.9, method='newton')
        p_4 = result.root
        state_4 = Fluid.set_state(ml.PSmass_INPUTS, p_4, state_01.s)
        u_4 = omega * radius
        v_4 = mass_flow / (state_4.d * area * ml.cosd(angle))
        v_t_4 = v_4 * ml.sind(angle)
        v_m_4 = v_4 * ml.cosd(angle)
        w_t_4 = v_t_4 - u_4
        w_m_4 = v_m_4
        ...
        p_4_array[i] = p_4
        Ma_4_array[i] = Ma_4
        state_03 = Fluid.set_state(ml.HmassSmass_INPUTS, h0rel_in, state_01.s)
        d_crit = state_03.d * ((gamma + 1) / 2) ** (-1 / (gamma-1))
        a_crit = state_03.a * ((gamma + 1) / 2) ** (-0.5)
        mass_flux_crit_3[i] = d_crit*a_crit
        mass_flow_crit_3[i] = d_crit*a_crit*area*ml.cosd(angle)
        ...
        result = root_scalar(res, x0=p_2, method='newton')
        p_5 = result.root
        state_5 = Fluid.set_state(ml.PSmass_INPUTS, p_5, state_01.s)
        u_5 = omega * radius
        w_5 = mass_flow / (state_5.d * area * ml.cosd(angle))
        w_t_5 = w_5 * ml.sind(angle)
        w_m_5 = w_5 * ml.cosd(angle)
        v_t_5 = w_t_5 + u_5
        v_m_5 = w_m_5
        ...
        state_04rel = Fluid.set_state(ml.HmassSmass_INPUTS, h0rel_in, state_01.s)
        d_crit = state_04rel.d * ((gamma + 1) / 2) ** (-1 / (gamma-1))
        a_crit = state_04rel.a * ((gamma + 1) / 2) ** (-0.5)
        mass_flux_crit_4[i] = d_crit*a_crit
        mass_flow_crit_4[i] = d_crit*a_crit*area*ml.cosd(angle)
