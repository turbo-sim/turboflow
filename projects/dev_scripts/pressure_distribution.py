...
def get_enthalpy_residual_outlet(
    p_out, h0_rel_in, s_in, mass_flow, area_out, angle_out
):
...
for i, mass_flow in enumerate(mass_flows):
    ...
    res = lambda p: get_enthalpy_residual_inlet(
        p, state_01.h, state_01.s, mass_flow, area_1
    )
    ...
    res = lambda p: get_enthalpy_residual_outlet(
        p, h0rel_in, state_01.s, mass_flow, area, angle
    )
...
ax.plot(
    PR, Ma_3_array, linewidth=1.25, label="Rotor 1 exit", marker=marker, color=colors[1]
)
ax.plot(
    PR,
    Ma_4_array,
...
ax.plot(
    PR, Ma_5_array, linewidth=1.25, label="Rotor 2 exit", marker=marker, color=colors[3]
)
...
ax.plot(
    PR, p_2_array, linewidth=1.25, label="Stator 1 exit", marker=marker, color=colors[0]
)
ax.plot(
    PR, p_3_array, linewidth=1.25, label="Rotor 1 exit", marker=marker, color=colors[1]
)
ax.plot(
    PR, p_4_array, linewidth=1.25, label="Stator 2 exit", marker=marker, color=colors[2]
)
ax.plot(
    PR, p_5_array, linewidth=1.25, label="Rotor 2 exit", marker=marker, color=colors[3]
)
leg = ax.legend(loc="best")
fig.tight_layout(pad=1, w_pad=None, h_pad=None)
# ax.set_ylim([58, 60.1])
