...
for speed  in speed_percent:
    ...
    mass_flow_rate_exp = data_exp["Mass flow rate"][data_exp["Mass flow rate"]["omega"] == speed]["m"].values
    mass_flow_rate_sim = data_mass_flow[(data_mass_flow["speed_percent"] > speed-1) & (data_mass_flow["speed_percent"] < speed+1)]["mass_flow_rate"].values
    ...
    torque_exp = data_exp["Torque"][data_exp["Torque"]["omega"] == speed]["Torque"].values
    torque_sim = data_torque[(data_torque["speed_percent"] > speed-1) & (data_torque["speed_percent"] < speed+1)]["torque"].values
    ...
    alpha_exp = np.cos(data_exp["alpha_out"][data_exp["alpha_out"]["omega"] == speed]["alpha_out"].values*np.pi/180)
    alpha_sim = np.cos(data_alpha[(data_alpha["speed_percent"] > speed-1) & (data_alpha["speed_percent"] < speed+1)]["exit_flow_angle"].values*np.pi/180)
