...
def find_critical_mass_flux(Ma_crit, p0, T0, fluid):
    ...
    def get_residual(v):
        h = h0 - v**2 / 2
        props = fluid.set_state(cp.HmassSmass_INPUTS, h, s0 * fac)
        res = Ma_crit - v / props["a"]
        return res

    # Compute the velocity corresponding to critical Mach
    props_in = fluid.set_state(cp.PT_INPUTS, p0, T0)
    s0 = props_in["s"]
    h0 = props_in["h"]
...
def get_Aungier_beta(v_out, p0, T0, fluid, opening, pitch, radius_curvature, Ma_crit):
    ...
    delta_0 = (
        np.arcsin(opening / pitch * (1 + (1 - opening / pitch) * (beta_g / 90) ** 2))
        * 180
        / np.pi
        - beta_g
    )
...
def get_beta_supersonic(v_out, p0, T0, fluid, phi_crit):
    ...
    props_out = fluid.set_state(cp.HmassSmass_INPUTS, h_out, s_out * fac)
...
def polynomial_blending_asymmetric(f1, f2, x0, x, a, alpha):
    ...
    coefficients = np.array([[1, 1, 1], [2, 3, 4], [a**2, a**3, a**4]])
    ...
    sigma = (
        0
        + (solution[0] * xx**2 + solution[1] * xx**3 + solution[2] * xx**4)
        * (xx > 0)
        * (xx < 1)
        + 1 * (xx > 1)
    )
...
fluid_name = "air"
Fluid = tf.Fluid(fluid_name, "HEOS")
...
p0 = 101325
T0 = 300
stagnation_props = Fluid.set_state(cp.PT_INPUTS, p0, T0)
...
beta_inc = 58
f_subsonic = lambda Ma: beta_inc + (beta_crit - beta_inc) * (Ma - Ma_inc) / (
    Ma_crit - Ma_inc
)
...
for v in v_out:
    ...
    if Ma_exit[-1] < Ma_crit * k1:
        ...
        beta_blended.append(
            sigmoid_blending_asymmetric(
                beta_subsonic[-1], beta_supersonic[-1], x, n=3, m=0.5
            )
        )
    else:
        beta_blended.append(beta_supersonic[-1])
    h = h0 - 0.5 * v**2
    static_props = Fluid.set_state(cp.HmassSmass_INPUTS, h, s0)
    d = static_props["d"]
    rho.append(d)
    phi.append(v * d)
